...
object FullPositiveSuperCompilerApp {
  val help = """usage: hosc.exp.FullPositiveSuperCompilerApp -si sinput_file -t tree_output_file -p program_output_file
  |Where:
  ...
  def main(args : Array[String]) : Unit = {
    ...
    val sc = new FullPositiveSuperCompiler(program)
    ...
    fw.write("-- generated by higher-level SC from " + fileName + "\n")
    doc.format(120, fw)
    fw.flush();
    fw.close();
  }
