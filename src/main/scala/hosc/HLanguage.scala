...
object HLanguage {
   ...
   case class LetExpression(bs: List[(Variable, Expression)], expr: Expression) extends Expression {
     ...
     def \\(s: Map[Variable, Variable]) = LetExpression(bs map {b => ((b._1\\s), (b._2\\s).asInstanceOf[Expression])}, expr\\s);
     def / (s: Map[Variable, Expression]) = LetExpression(bs map {b => (b._1, (b._2/s))}, expr/s)
