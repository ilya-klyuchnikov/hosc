...
object LambdaLifting {
  ...
  def findLetRec(e: Expression): Option[LetRecExpression] = e match {
    ...
    case Constructor(_, args) =>
      args.foldLeft(None: Option[LetRecExpression]) { (r, expr) => r.orElse(findLetRec(expr)) }
  ...
  private def injectArgument(e: Expression, f: Variable, v0: Variable): Expression = e match {
    ...
    case LetRecExpression((f1, e1), e2) =>
      if (f1 == f) {
        val nv = newVar()
        LetRecExpression(
          (f1, LambdaAbstraction(nv, injectArgument(e1, f, v0) \\ Map(v0 -> nv))),
          injectArgument(e2, f, v0),
        )
      } else {
  ...
  private def extractGlobals(e1: Expression, p: ListBuffer[Function]): Expression = e1 match {
    ...
    case CaseExpression(sel, bs) =>
      CaseExpression(extractGlobals(sel, p), bs map { b => Branch(b.pattern, extractGlobals(b.term, p)) })
