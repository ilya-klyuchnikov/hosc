...
class LemmaGenerator(val funs: List[Variable], val cons: List[Ctr]) {
  ...
  def genAllExprs(maxSize: Int, vars: List[Variable]): List[Expression] =
    (List.range(1, maxSize + 1) map {genExps(_, vars)}).flatten
  ...
  private def genExps(size: Int, vars: List[Variable]): List[Expression] =  size match {
    ...
    case n => {
      ...
      val ctrLs = for (ctr <- cons) yield {
        ...
        (ps map {combExps(_, vars)}).flatten map {Constructor(ctr.name, _)}
      }
      return appLs.flatten ++ ctrLs.flatten
    }
  }
...
object LemmaGenerator {
  ...
  def apply(program: Program) = {
    ...
    val cons: List[Ctr] = (program.ts map {_.cons}).flatten map {dc => Ctr(dc.name, dc.args.length)}
  ...
  def parts(size:Int, arity:Int): List[List[Int]] =
    ...
    else (List.range(1, size - arity + 2) map {x => parts(size - x, arity - 1) map {x::_}}).flatten
}
