...
-----------------------------------------------------------------
-- This file is slightly modified from Language.Haskell.TH.PprLib
-----------------------------------------------------------------
...
import Language.Haskell.TH.Syntax (Name(..), NameFlavour(..),nameBase)
import qualified Text.PrettyPrint.HughesPJ as HPJ
import Monad (liftM, liftM2)
-- import Data.FiniteMap (FiniteMap, lookupFM, emptyFM, addToFM)
import qualified Data.Map (Map, lookup, empty, insert)
import GHC.Base (Int(..))
...
type State = (Data.Map.Map Name HPJ.Doc, Int)
data PprM a = PprM { runPprM :: State -> (a, State) }

pprName :: Name -> Doc
pprName n@(Name o (NameU _))
 = PprM $ \s@(fm, i@(I# i'))
        -> case Data.Map.lookup n fm of
               Just d -> (d, s)
               Nothing -> let d = HPJ.text $ show $ Name o (NameU i')
                          in (d, (Data.Map.insert n d fm, i + 1))
pprName n = text $ nameBase n
...
to_HPJ_Doc d = fst $ runPprM d (Data.Map.empty, 0)
