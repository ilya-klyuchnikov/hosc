--------------------------------------------------------------
-- This file is slightly modified from Language.Haskell.TH.Ppr
--------------------------------------------------------------

-- TH.Ppr contains a prettyprinter for the
-- Template Haskell datatypes

module Yicho.Ppr(pprint,Ppr(ppr)) where
...
isOperatorName name = and [ n `elem` ".:!#$%&*+/<=>?@^|-\\" | n <- nameBase name ]

isOperator :: Exp -> Bool
isOperator (VarE v) = isOperatorName v
isOperator (ConE c) = isOperatorName c
isOperator _        = False

pprExp' :: Precedence -> Exp -> Doc
pprExp' i e@(VarE v) = parensIf (isOperatorName v) (pprExp opPrec e)
pprExp' i e@(ConE v) = parensIf (isOperatorName v) (pprExp opPrec e)
pprExp' i e          = pprExp i e

pprExp :: Precedence -> Exp -> Doc
pprExp _ (VarE v)     = pprName v
pprExp _ (ConE c)     = pprName c
pprExp i (LitE l)     = pprLit i l
-- pprExp i (AppE e1@(VarE v1) e2) =
--     parensIf (i >= appPrec) $ (parensIf (isOperatorName v1) (pprExp opPrec e1))
--                  <+> pprExp appPrec e2
-- pprExp i (AppE e1@(ConE v1) e2) =
--     parensIf (i >= appPrec) $ (parensIf (isOperatorName v1) (pprExp opPrec e1))
--                  <+> pprExp appPrec e2
...
pprExp i (InfixE (Just e1) op (Just e2))
 = parensIf (i >= opPrec) $ pprExp opPrec e1
                        <+> (if isOperator op then pprExp noPrec op
                             else text "`" <> pprExp noPrec op <> text "`") -- e `foldr (+)` [] is OK?
                        <+> pprExp opPrec e2
pprExp i (InfixE Nothing op Nothing) = pprExp i op -- Ex. let a = [| foldr (+) |] in infixE Nothing a Nothing
pprExp _ (InfixE me1 op me2) = parens $ pprMaybeExp noPrec me1
                               <+> (if isOperator op then pprExp noPrec op
                                    else text "`" <> pprExp noPrec op <> text "`")
