...
data Boolean = True | False | False1 | False2 | False3 | False4 | False5 | False6 | False7;
data List a = Nil | Cons a (List a);

(chState2 (loop1 (State (S x) Z Z Z Z Z Z Z Z) (f s))) where

loop1 = \state actions -> case actions of {
	Nil -> state;
	Cons a as -> case (react state a) of {
		State i sU eU eM pR pW pEMR pEMW pSU  -> loop1 (State i sU eU eM pR pW pEMR pEMW pSU) as;
	};
};
...
r2 = \i sU eU eM pR pW pEMR pEMW pSU -> 
	case i of {S i1 -> case pW of {Z -> 
	State i1 Z Z Z (S pR) Z (add pEMR eM) pEMW (add sU (add eU pSU))
;};};
...
r5 = \i sU eU eM pR pW pEMR pEMW pSU -> 
	case pR of {S pR1 -> case pR1 of { S pR2 -> case pSU of {Z -> case pEMR of { Z -> 
	State i (S (S (add sU pR2))) eU eM Z pW Z pEMW Z;
};};};};
r6 = \i sU eU eM pR pW pEMR pEMW pSU -> 
	case pR of {S pR1 -> case pEMR of {Z -> case pSU of { Z ->    
	State i sU (S eU) eM pR1 pW Z pEMW Z;
};};};
