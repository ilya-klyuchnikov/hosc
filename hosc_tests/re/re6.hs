...
task1c = \w -> match (or nil (rep a)) w;
task1d = \w -> match (rep (or nil a)) w;

-- aa* | nil == a*
task2a = \w -> match (or nil (concat a (rep a))) w;
task2b = \w -> match (rep a) w;

-- a*a* == a*
task3a = \w -> match (concat1 (rep a) (rep a)) w;
...
task7b = \w -> match (rep (concat1 (rep a) (rep b))) w;  
